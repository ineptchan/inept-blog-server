...
interface ArticleRepository : JpaRepository<Article, Long> {
    ...
    @Query("SELECT NEW top.inept.blog.feature.article.pojo.dto.ArticleTitleDTO(a.id, a.title) FROM Article a WHERE a.id IN :articleIds")
    fun findTitleAllById(articleIds: List<Long>): List<ArticleTitleDTO>

    @Query("SELECT NEW top.inept.blog.feature.article.pojo.dto.ArticleTitleDTO(a.id, a.title) FROM Article a WHERE a.id = :articleId")
